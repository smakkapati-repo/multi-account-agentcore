#!/usr/bin/env python3
"""
Setup Child2-Demo Account Infrastructure
- S3 bucket for trade risk documents
...
# AWS profile for child2-demo
PROFILE = "child2-demo"

# Resource names
S3_BUCKET_NAME = f"child2-trade-kb-{CHILD2_ACCOUNT_ID}"
IAM_ROLE_NAME = "CentralAccountAccessRole"
OPENSEARCH_COLLECTION_NAME = "trade-risk-kb"

def create_s3_bucket(s3_client):
    """Create S3 bucket for trade risk documents"""
...
def upload_data_to_s3(s3_client, bucket_name):
    """Upload trade risk data to S3"""
    print(f"\n📤 Uploading data to S3...")
    
    data_dir = Path("data/child2_trade_risk")
    if not data_dir.exists():
        print(f"  ⚠️  Data directory not found: {data_dir}")
        print(f"  ℹ️  Run fetch_trade_data.py first")
        return False
...
def create_iam_role(iam_client):
    ...
    try:
        response = iam_client.create_role(
            ...
            Description="Allows central account to access Child2-Demo resources",
        ...
        "Statement": [
            {
                ...
                "Resource": f"arn:aws:aoss:us-east-1:{CHILD2_ACCOUNT_ID}:collection/*"
            },
            {
                "Effect": "Allow",
                "Action": [
...
def create_opensearch_collection(aoss_client):
    ...
    try:
        ...
        response = aoss_client.create_collection(
            ...
            type='VECTORSEARCH',
            description='Trade risk knowledge base for Child2-Demo LOB'
        )
...
def save_config(bucket_name, role_arn, collection_id):
    ...
    config_file = Path("infra/child2_config.json")
...
def main():
    print("=" * 70)
    print("Child2-Demo Account Infrastructure Setup")
    print(f"Account: {CHILD2_ACCOUNT_ID} (profile: {PROFILE})")
    print("=" * 70)
    ...
    try:
        ...
        if current_account != CHILD2_ACCOUNT_ID:
            print(f"❌ Error: Expected account {CHILD2_ACCOUNT_ID}, but got {current_account}")
    ...
    print("\n" + "=" * 70)
    print("✅ Child2-Demo Infrastructure Setup Complete!")
    print("=" * 70)
    print(f"\n📦 S3 Bucket: {bucket_name}")
    print(f"🔐 IAM Role: {role_arn}")
    if collection_id:
        print(f"🔍 OpenSearch Collection: {collection_id}")
    print(f"\n💾 Configuration saved to: infra/child2_config.json")
    print("\n🔜 Next steps:")
    print("   1. Wait for OpenSearch collection to be ACTIVE (3-5 minutes)")
    print("   2. Run: python scripts/embed_child2_data.py")
